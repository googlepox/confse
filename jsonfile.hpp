...
namespace objson
{
    ...
    static std::map<std::filesystem::path, nlohmann::json> file_cache;

    // the game directory should be the one below which all accessed files reside
    static std::filesystem::path game_directory;
    ...
    public:
        ...
        jsonfile(std::string filename)
        {
            ...
            if ((it) != file_cache.end())
            {
                ...
                filedata = file_cache.at(filepath);
            }
        ...
        template<typename T>
        T get_entry(std::string key, T value)
        {
        ...
        void flush_to_disk(bool evict_from_cache = false)
        {
            if (allow_disk_access)
            {
                ...
                auto it = file_cache.find(filepath);
                if (evict_from_cache && it != file_cache.end())
                {
                    file_cache[filepath] = filedata;
                }
            }
        }
    };
